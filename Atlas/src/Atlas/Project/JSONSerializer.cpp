...
std::unique_ptr<EntitySnapshot> JSONSerializer::saveEntitySnapshot(Entity entity) {
	std::unique_ptr<JSONEntitySnapshot> snapshot = std::make_unique<JSONEntitySnapshot>();
	serializeEntity(entity, snapshot->data);
	return std::move(snapshot);
}

void JSONSerializer::recallEntitySnapshot(Entity& dst, const EntitySnapshot& snapshot) {
	deserializeEntity(dst, static_cast<const JSONEntitySnapshot&>(snapshot).data);
}
...
void JSONSerializer::deserializeEntity(Entity& entity, const json& data) {
	if (data.contains("Transform")) {
		Component::Transform& transform = entity.getComponent<Component::Transform>();
		transform.position				= {data["Transform"]["Position"][0], data["Transform"]["Position"][1], data["Transform"]["Position"][2]};
		transform.size					= {data["Transform"]["Size"][0], data["Transform"]["Size"][1]};
		transform.rotation				= data["Transform"]["Rotation"];
	}

	if (data.contains("Sprite")) {
		std::string texPath = ProjectManager::toAbsolutePath(data["Sprite"]["Texture"]);
		AssetManager::loadTexture(texPath);
		glm::ivec2 index	   = {data["Sprite"]["Index"][0], data["Sprite"]["Index"][1]};
		glm::vec2  sizeInTiles = {data["Sprite"]["Size"][0], data["Sprite"]["Size"][1]};
	...
	if (data.contains("Script")) {
		std::string scriptName = data["Script"]["Name"];
